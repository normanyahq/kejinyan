...
def getDiskUsage():
    command = '''df -t $PWD | tail -n 1 | awk {'print $5'}'''
...
def get_db():
    ...
    db = connect(database='postgres', host='localhost',
                 user='heqingy', password='heqingy')
    return db
...
def convert_and_recognize(token, paths, answersheet_type):
    ...
    if standard['status'] == "error":
        c.execute("insert into error_list values (%s, %s, %s);",
...
def returnTable(token):
    ...
    if not os.path.isfile(xlsx_path):
        ...
        # hack
        cur.execute("select type from status where token = %s;", (token, ))
        ansType = cur.fetchone()[0]
        ...
        credits = ([1] * 25 + [0] * 5 + [1] * 15 +
                   [2] * 15) if ansType == 'full_4option' else None
        utility.excel.generateXlsx(
            xlsx_path, standard['answer'], answers, credits)
        db.close()
...
def getProgress(token):
    ...
    cur = db.cursor()
    cur.execute(
        "select processed, total from status where token = %s;", (token, ))
    t = cur.fetchone()
    db.close()
...
def get_results(token):
    ...
    if t:
        ...
        status = t['status']
        if status == "error":
            return u"标准答案识别出错，请确认答题卡类型选择正确，且答题卡扫描件的三个定位块完整清晰；可尝试重新扫描上传。如仍有问题，\
...
def upload_file():
    ...
    if request.method == "GET" and request.values.get('upload') == 'true':
        ...
        db = get_db()
        c = db.cursor()
        c.execute("insert into status (token, processed, total, type) values (%s, 0, %s, %s);",
                  (token, 1, request.values['answersheettype']))
        db.commit()
